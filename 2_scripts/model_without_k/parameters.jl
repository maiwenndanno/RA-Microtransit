...
function create_params(tsnetwork,locs_id, model_inputs, wo, wd,I,t,tstep,horizon,benchmark)
    ...
    Ia=create_Ia(I,A,Ai);
    P,P_T,P_desc = create_paths(vo,vd,I,locs_id,wo,wd,benchmark);
    O, D, H, H_times = create_OHD_sets(N,I,P,t,gamma,tsnetwork.nodedesc,tstep,wo, wd,deadlines,tsnetwork.shortest_time, horizon,P_desc);
    Ai_except_H,Ai_except_H_desc=create_Ai_except_H(Ai,I,P,tsnetwork,P_desc);
    N_vo, N_vd, N_except_o_d_H=create_N_vo_vd_H(N,I,P, P_desc,H,tsnetwork.nodedesc);
    return (vo=vo, vd=vd, P=P, H=H, H_times=H_times, O=O, D=D, deadlines=deadlines,
            N_vo=N_vo, N_vd=N_vd, N_except_o_d_H=N_except_o_d_H, N_except_sink=N_except_sink,
            P_T=P_T, P_desc=P_desc, A=A, Ai=Ai, Ai_plus=Ai_plus, Ai_minus=Ai_minus, Ai_minus_tilde=Ai_minus_tilde, Ia=Ia, Ai_except_H=Ai_except_H, Ai_except_H_desc=Ai_except_H_desc,
...
function create_paths(vo,vd,I,locs_id,wo,wd,benchmark)
    ...
    P_desc=Dict() # Description of each path of each cust, key (i,p)
    for i in I
        P[i]=Vector()
        P_T[i]=Vector()
        P_desc[i]=Dict()
...
function create_OHD_sets(N,I,P,t,gamma,nodedesc,tstep,wo, wd,deadlines, shortest_time, horizon,P_desc)
    # Wk: max malking time
    ...
    for i in I
        ...
        for p in P[i]
            ...
            for n in N
                node=nodedesc[n]
                if node[1] in P_desc[i][p]["o"] && (node[2]<=deadlines[i]-(gamma[i]["best_full"]-wo[i,node[1]])) && (node[2]>=t[i]+wo[i,node[1]])
                    # gamma[i]["best_full"]-wo[i,node[1]] is a lower bound of the remaining travel time from this pick-up vbs
                    push!(O_ip, n)
                end
                if node[1] in P_desc[i][p]["d"] && (node[2]<=deadlines[i]-wd[i,node[1]]) && (node[2]>=tstep * ceil((t[i]+wo[i,gamma[i]["best_vo"]])/tstep)+shortest_time[gamma[i]["best_vo"],node[1]])
                    push!(D_ip, n)
                else
                    if P_desc[i][p]["transfer"]==1 # if transfer
                        if node[1] in P_desc[i][p]["h"] 
                            push!(H_ip, n)
...
function create_N_vo_vd_H(N,I,P, P_desc,H_ind,nodedesc)
    ...
    N_except_o_d_H=Dict() # dictionnaire of nodes except pick-up and drop-off locations
    for i in I
        N_vo[i]=Vector()
        N_vd[i]=Vector()
        N_except_o_d_H[i]=Dict()
        for p in P[i]
            N_except_o_d_H[i][p]=Vector()
            for n in N
                node=nodedesc[n]
                if node[1] == P_desc[i][p]["o"]#in vo[i]
                    if !(n in N_vo[i])
                        push!(N_vo[i], n)
                    end
                elseif node[1] == P_desc[i][p]["d"]#in vd[i]
                    if !(n in N_vd[i])
                        push!(N_vd[i], n)
                    end
                elseif !(node[1] in H_ind[i][p])
                    push!(N_except_o_d_H[i][p], n)
                end
            end
        end
    end
    return N_vo, N_vd, N_except_o_d_H
end

# -----------------------------------------------------------------------------------#
function create_Ai_except_H(Ai,I,P,tsnetwork,P_desc)
    # Remove from Ai any arcs that is stationnary at a hub location of the select path p
    Ai_except_H=Dict()
    Ai_except_H_desc=Dict()
    for i in I
        Ai_except_H[i]=Dict()
        Ai_except_H_desc[i]=Dict()
        for p in P[i]
            index=1
            Ai_except_H[i][p]=Vector()
            Ai_except_H_desc[i][p]=Dict()
            if P_desc[i][p]["transfer"]==0
                for a in Ai[i] # We do not remove any arcs
                    push!(Ai_except_H[i][p],index)
                    Ai_except_H_desc[i][p][index]=a
                    index+=1
                end
            else 
                hubs=P_desc[i][p]["h"]
    ...
    return Ai_except_H,Ai_except_H_desc
end
...
# -----------------------------------------------------------------------------------#
# Return the start node index of an arc (index type)
function get_n1(a,tsnetwork)
    return tsnetwork.arcdesc[a][1]
end

# -----------------------------------------------------------------------------------#
# Return the end node index of an arc (index type)
function get_n2(a,tsnetwork)
    return tsnetwork.arcdesc[a][2]
end
