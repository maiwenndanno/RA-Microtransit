...
function get_resultfolder(model_type,map_type,benchmark)
...
function save_and_display(map1, model_type_results,map_type,benchmark,save_res,sol,horizon,tstep,tsnetwork,params,abbrev,data,print_all,benders_prop=nothing)
    resultfolder=get_resultfolder(model_type_results,map_type,benchmark)
    if save_res
    ...
    map2bis=print_traveling_arcs(sol,0,map1,data.locs,tsnetwork,params,horizon,data.locs_id,data.locs_desc,print_all,save_res,resultfolder)
    if model_type_results=="benders" || model_type_results=="benders_test" && benders_prop !== nothing
        plot_bounds, plot_times = print_benders(print_all,save_res,resultfolder,benders_prop)
    else
        plot_bounds, plot_times = nothing, nothing
    end
    ...
    for k in 1:1:min(10,benders_prop["n_it"])
        sol_k=(x=benders_prop["x_SP_all"][k],xi=benders_prop["xi_SP_all"][k],z=benders_prop["z_MP_all"][k]);
        timespaceviz_bus(resultfolder*string(k)*"it) tsplot.png", horizon, tstep, tsnetwork, params, sol_k.z, data.locs_id, data.locs_desc,x_size=2000, y_size=1000)
    end
    return map2bis, plot_bounds, plot_times
end