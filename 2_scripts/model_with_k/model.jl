...
function network_model(Q,abbrev,wo,tsnetwork,params,coefficients,output=1)
    ...
    P, H, O, D, N_vo, N_vd, N_except_o_d_H, P_T= params.P, params.H, params.O, params.D, params.N_vo, params.N_vd, params.N_except_o_d_H, params.P_T;
    ...
    @constraint(model, [i in I], sum(x[i,a,p,k] for p in P[i], n in O[i][p], a in Ai_plus[i][n], k in K) <= 1) #each customer is assigned to max one arc leaving a pick-up node from the selected path
    @constraint(model, [i in I], sum(x[i,a,p,k] for p in P[i], n in D[i][p], a in Ai_minus[i][n], k in K) == sum(x[i,a,p,k] for p in P[i], n in O[i][p], a in Ai_plus[i][n], k in K)) #each customer is assigned to exactly one arc incoming a drop-off node from the selected path if he is served
    ...
    @constraint(model, [i in I, p in P[i], n in N_except_o_d_H[i][p],k in K], sum(x[i,a,p,k] for a in Ai_minus[i][n]) == sum(x[i,a,p,k] for a in Ai_plus[i][n]))
...
function network_model_OLD(I,K,Q,q,t,wo,tsnetwork,params,coefficients)
    ...
    A_plus, A_minus, arccost,nodedesc, arcdesc =tsnetwork.A_plus, tsnetwork.A_minus, tsnetwork.arccost,tsnetwork.nodedesc, tsnetwork.arcdesc;
    P, H, O, D, N_vo, N_vd, N_except_o_d_H, P_T, A, N, N_star, A_tilde_depot= params.P, params.H, params.O, params.D, params.N_vo, params.N_vd, params.N_except_o_d_H, params.P_T, params.A, params.N, params.N_star, params.A_tilde_depot;
    c=arccost #  duration of the arc
    ...
    # Customer flow balance
    @constraint(model, [i in I, p in P[i], n in N_except_o_d_H[i][p],k in K], sum(x[i,a,p,k] for a in A_minus[n]) == sum(x[i,a,p,k] for a in A_plus[n]))
