...
function network_model(Q,abbrev,wo,tsnetwork,params,coefficients,output=1)
    ...
    P, H, O, D, N_vo, N_vd, N_except_vo_vd_H, N_except_sink, P_T, H_times= params.P, params.H, params.O, params.D, params.N_vo, params.N_vd, params.N_except_vo_vd_H, params.N_except_sink, params.P_T, params.H_times;
    A, Ai, Ai_plus, Ai_minus, Ai_minus_tilde, Ia, N, N_star, A_tilde_depot, Ai_except_H=params.A, params.Ai, params.Ai_plus, params.Ai_minus, params.Ai_minus_tilde, params.Ia, params.N, params.N_star, params.A_tilde_depot, params.Ai_except_H;
    ...
    # Pick-up
    @constraint(model, [i in I], sum(x[i,a,p] for p in P[i], n in O[i][p], a in Ai_plus[i][n]) == 1) #each customer is assigned to exactly one arc leaving a pick-up node from the selected path
    # Drop-off
    @constraint(model, [i in I], sum(x[i,a,p] for p in P[i], n in D[i][p], a in Ai_minus[i][n]) == 1) #each customer is assigned to exactly one arc incoming a drop-off node from the selected path
    # Transfer
    ...
    @constraint(model, [i in I, p in P[i], n in N_except_vo_vd_H[i][p]], sum(x[i,a,p] for a in Ai_minus[i][n]) == sum(x[i,a,p] for a in Ai_plus[i][n]))
    ...
    @constraint(model, [i in I, p in P[i], a in Ai_except_H[i][p]], x[i,a,p] <= z[a])
...
##------    
function network_model_z_xifixed(Q,abbrev,wo,tsnetwork,params,coefficients,z,xi,output=1)
    # Create a model
    model = Model(Gurobi.Optimizer)
    set_optimizer_attribute(model, "OutputFlag", output)
    #set_optimizer_attribute(model, "MIPGap", 0.1)
    ...
    solution = (xi,x=value.(x),z, obj=objective_value(model),time=solvetime,objs=objs)
